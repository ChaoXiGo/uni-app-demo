...
import { getGoodsDataAPI } from '@/services/category'
import type { Details, GoodsResult } from '@/types/category'
...
// 商品详情
const goodsDetails = ref<Details>()
// 同类商品
const similarGoods = ref<GoodsItem[]>()
const getGoodsData = async () => {
  const res = await getGoodsDataAPI(query.id)
  goodsData.value = res.result
  goodsDetails.value = res.result.details
  similarGoods.value = res.result.similarProducts
}
      ...
      <view class="preview">
        <swiper circular @change="onChange">
          <swiper-item v-for="item in goodsData?.mainPictures" :key="item">
            <image mode="aspectFill" :src="item" @tap="onClick(item)" />
          </swiper-item>
        </swiper>
    ...
    <view class="detail panel">
      ...
      <view class="content">
        <view class="properties">
          <!-- 属性详情 -->
          <view class="item" v-for="item in goodsDetails?.properties" :key="item.name">
        ...
        <!-- 图片详情 -->
        <image
          v-for="item in goodsDetails?.pictures"
          :key="item"
          mode="widthFix"
          :src="item"
        ></image>
